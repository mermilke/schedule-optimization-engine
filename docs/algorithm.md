...
~80 players sign up for 49 slots. Each player picks a preferred time and some alternates. Each player has a "speedups" number (their contribution value -- higher is better). I want the 49 players with the most speedups to get slots, and I want as many of them as possible to actually fit.
...
It's 49 slots and ~80 players. The whole thing runs in under a second. The real bottleneck is Google Sheets API calls (reading/writing cells), not the algorithm.
